...
const CODEPAGE = 0; // CP437
const ENCODING = 'cp437';
const CHARS_PER_LINE = 42;
const CUT_COMMAND = Buffer.from([0x1b, 0x69]);
const EJECT_COMMAND = Buffer.from([0x1d, 0x65, 0x05]);
const BARCODE_DPI = 200;

export class CustomVKP80Adapter extends PrinterAdapter {
  readonly manufacturer = 'Custom';
  readonly model = 'VKP-80';
  ...
  async printReceipt(data: ReceiptData): Promise<PrintResult> {
    ...
    try {
      ...
      for (const line of data.lines) {
        switch (line.type) {
          ...
          case 'barcode': {
            ...
            const barcode = await renderBarcode(line.value, line.format ?? 'EAN8', {
              dpi: BARCODE_DPI,
              moduleWidth: 0.6,
              moduleHeight: 16,
              includeText: true,
      ...
      if (data.cutAfter) {
        // VKP-80 cut + eject sequence
        parts.push(CUT_COMMAND);
        parts.push(EJECT_COMMAND);
      }
  ...
  async cutPaper(): Promise<void> {
    ...
    await this.connection.write(Buffer.concat([CUT_COMMAND, EJECT_COMMAND]));
  }
